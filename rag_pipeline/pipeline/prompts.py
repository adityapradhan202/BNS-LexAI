...
augment_generate_prompt = ChatPromptTemplate.from_messages(
        messages=[
            ...
            ("human", "Use the details of the provided context and give an answer in markdown format!"),
            ("human", "{query}")
        ]
)
...
context_relevancy_prompt = ChatPromptTemplate.from_template(
    """
    You have to check relevancy of context.
    ...
    If the answer to the query is present in the context, classify context into 'relevant', otherwise classify it into 'irrelevant'.
    """
)
